...
export default function GeneratePage() {
  ...
  return (
    <main className="min-h-screen bg-zinc-50 text-gray-800 p-8">

      {/* Header */}
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-bold mb-1 text-emerald-700">PromptData</h1>
          <p className="text-gray-500">Describe your dataset. Get it instantly.</p>
        </div>
        <button
          onClick={() => setShowHistory(!showHistory)}
          className="flex items-center gap-2 bg-white hover:bg-emerald-100 border border-emerald-200 px-4 py-2 rounded-lg text-sm transition-colors text-gray-700 shadow-sm"
        >
          🕘 History {history.length > 0 && (
            <span className="bg-emerald-600 text-white text-xs px-2 py-0.5 rounded-full">{history.length}</span>
          )}
        </button>
      </div>
            ...
            <div className="space-y-2">
              {history.map((h, i) => (
                <div
                  ...
                  className="flex items-center justify-between bg-emerald-50 hover:bg-emerald-100 border border-emerald-100 rounded-lg px-4 py-2 cursor-pointer transition-colors"
                >
                  <p className="text-sm text-gray-700 truncate max-w-xl">{h.prompt}</p>
                  <div className="flex items-center gap-3 ml-4 shrink-0">
                    <span className="text-xs text-emerald-600 font-medium">{h.rows} rows</span>
                    <span className="text-xs text-teal-600 font-medium">{h.columns} cols</span>
        ...
        <div className="flex flex-wrap gap-2">
          {PRESETS.map((preset) => (
            <button
              ...
              className="bg-white hover:bg-emerald-100 border border-emerald-200 hover:border-emerald-400 text-sm px-4 py-1.5 rounded-full transition-colors text-gray-700 shadow-sm"
            >
              {preset.label}
            </button>
          ))}
      ...
      <div className="bg-white border border-emerald-200 rounded-2xl p-6 mb-6 shadow-sm">
        <textarea
          className="w-full bg-emerald-50 rounded-xl p-4 text-gray-800 border border-emerald-200 focus:border-emerald-500 outline-none resize-none h-32 mb-4 placeholder-gray-400"
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <select
            ...
            className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 text-sm text-gray-700 outline-none focus:border-emerald-500"
          >
            {LOCALES.map(l => (
          ...
          <button
            ...
            disabled={loading}
            className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-2 rounded-lg font-semibold disabled:opacity-50 transition-colors shadow-sm"
          >
            {loading ? (
      ...
      {loading && (
        <div className="mt-4 space-y-2 animate-pulse">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-10 bg-emerald-100 rounded-lg w-full" />
          ))}
        </div>
      )}
        ...
        <div className="mt-2">
          ...
          {Object.keys(numericStats).length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              {Object.entries(numericStats).slice(0, 4).map(([col, s]: any) => (
                <div key={col} className="bg-white border border-emerald-200 rounded-xl p-3 shadow-sm">
                  <p className="text-xs text-gray-400 mb-1 truncate">{col}</p>
                  <div className="flex justify-between text-xs">
                    <span className="text-blue-500 font-medium">min {s.min}</span>
                    <span className="text-emerald-600 font-medium">avg {s.avg}</span>
          ...
          <div className="flex items-center justify-between mb-3 gap-4">
            <div className="flex items-center gap-3">
              ...
              {sortCol && (
                <span className="text-xs text-gray-500">
                  Sorted by <span className="text-emerald-600 font-medium">{sortCol}</span> ({sortDir})
            ...
            <input
              ...
              className="bg-white border border-emerald-200 rounded-lg px-4 py-1.5 text-sm text-gray-700 outline-none focus:border-emerald-500 w-56 shadow-sm"
            />
          </div>
            ...
            <table className="w-full text-sm border-collapse">
                ...
                <tr>
                  {data.columns.map((col: string) => (
                    <th
                      ...
                      className="bg-emerald-100 p-3 text-left border-b border-emerald-200 text-emerald-800 cursor-pointer hover:bg-emerald-200 select-none whitespace-nowrap font-semibold"
                    >
              ...
              <tbody>
                {currentRows.map((row: any, i: number) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-white hover:bg-emerald-50' : 'bg-emerald-50 hover:bg-emerald-100'}>
                    {data.columns.map((col: string) => (
                      <td key={col} className="p-3 border-b border-emerald-100 text-gray-700 whitespace-nowrap">
                        {String(row[col])}
                      </td>
                    ))}
                  </tr>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setPage(0)} disabled={page === 0}
                className="px-3 py-1 rounded-lg bg-white border border-emerald-200 text-gray-600 hover:bg-emerald-100 disabled:opacity-30 text-sm shadow-sm">« First</button>
              <button onClick={() => setPage(p => p - 1)} disabled={page === 0}
                className="px-3 py-1 rounded-lg bg-white border border-emerald-200 text-gray-600 hover:bg-emerald-100 disabled:opacity-30 text-sm shadow-sm">‹ Prev</button>
              <span className="text-gray-500 text-sm px-2">Page {page + 1} of {totalPages}</span>
              <button onClick={() => setPage(p => p + 1)} disabled={page >= totalPages - 1}
                className="px-3 py-1 rounded-lg bg-white border border-emerald-200 text-gray-600 hover:bg-emerald-100 disabled:opacity-30 text-sm shadow-sm">Next ›</button>
              <button onClick={() => setPage(totalPages - 1)} disabled={page >= totalPages - 1}
                className="px-3 py-1 rounded-lg bg-white border border-emerald-200 text-gray-600 hover:bg-emerald-100 disabled:opacity-30 text-sm shadow-sm">Last »</button>
            </div>
          </div>
